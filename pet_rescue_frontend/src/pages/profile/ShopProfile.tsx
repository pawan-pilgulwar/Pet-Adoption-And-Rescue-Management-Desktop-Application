  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <h1 className="text-3xl font-black text-slate-800 mb-8">
        Shop <span className="text-teal-600">Profile</span>
      </h1>
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-orange-50 mb-6">
        <div className="flex items-center gap-4 mb-6 pb-6 border-b border-orange-50">
          <div className="w-16 h-16 rounded-full overflow-hidden bg-teal-100 border border-teal-200">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800">
              {profile?.shop_name || `${user.first_name} ${user.last_name}`}
            </h2>
            <p className="text-sm text-slate-500">@{user.username} • {user.email}</p>
            <span className="text-xs font-bold text-teal-600 bg-teal-50 px-2 py-0.5 rounded-full mt-1 inline-block uppercase">
              Pet Shop Owner
            </span>
          </div>
        </div>
        ...
        {message && (
          <div className={`p-3 rounded-xl mb-4 text-sm font-medium ${message.includes('successfully') ? 'bg-teal-50 text-teal-700 border border-teal-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
            {message}
          </div>
        )}
        ...
        {editMode ? (
          <div className="space-y-4">
            ...
            <div className="space-y-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
              ...
              <Input label="Shop Phone Number" name="phone_number" value={formData.phone_number} onChange={handleChange} />
              <Input label="Shop License" name="shop_license" value={formData.shop_license} onChange={handleChange} />
            </div>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm text-slate-700 font-medium">{profile?.shop_name || '—'}</p>
              </div>
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase">Shop Contact</p>
                <p className="text-sm text-slate-700 font-medium">{profile?.phone_number || '—'}</p>
              </div>
              <div className="col-span-full">
                <p className="text-xs font-bold text-slate-400 uppercase">Shop Address</p>
                <p className="text-sm text-slate-700 font-medium">{profile?.shop_address || '—'}</p>
              </div>
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase">License Number</p>
                <p className="text-sm text-slate-700 font-medium">{profile?.shop_license || '—'}</p>
              </div>
            </div>
            <div className="pt-3">
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-orange-50">
        ...
        {passwordMessage && (
          <div className={`p-3 rounded-xl mb-4 text-sm font-medium ${passwordMessage.includes('successfully') ? 'bg-teal-50 text-teal-700 border border-teal-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
            {passwordMessage}
          </div>
        )}
        <div className="space-y-4">
