...
const AdminPets: React.FC = () => {
  ...
  useEffect(() => {
    api.get('/pets/admin-all-pets/').then(r => setPets(r.data.data?.pets || [])).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-stone-900">Manage Pets</h1>
          <p className="text-stone-500 text-sm mt-0.5">Register and manage all pets in the system</p>
        </div>
      ...
      {loading ? <Spinner /> : pets.length === 0 ? <Empty icon="🐾" text="No pets in the system." /> : (
