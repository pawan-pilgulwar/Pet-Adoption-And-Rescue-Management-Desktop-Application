...
export default function ChatWidget() {
  ...
  // Reset active room when chat widget is closed to prevent automatic read-marking of new messages
  useEffect(() => {
    if (!isOpen) {
                ...
                <div className="p-4 bg-white border-b border-stone-100 flex items-center justify-between shadow-sm z-10 shrink-0">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className={`h-2 w-2 rounded-full ${isActiveOtherOnline ? 'bg-emerald-500 animate-pulse' : 'bg-stone-300'
                          }`} />
                  ...
                  {!activeRoom.is_completed && (
                    <button
                      ...
                      className={`text-xs font-bold px-3 py-1.5 rounded-full border transition-all duration-200 shadow-sm ${(activeRoom.reporter.id === user.id && activeRoom.reporter_marked_completed) ||
                  ...
                  {loadingMessages && messages.length === 0 ? (
                    ...
                    messages.map(msg => {
                      ...
                      return (
                        <div
                          key={msg.id}
                          className={`max-w-[70%] rounded-2xl px-4 py-2.5 text-sm shadow-sm relative group flex flex-col ${isMe
